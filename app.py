...
class ProfessionalPDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 10, f'Emitido por: {self.emitente} | Data: {self.data_emissao}', 0, 0, 'L')
...
def gerar_relatorio_final(orgao, emitente, matricula, membros, dados):
    ...
    pdf.cell(0, 15, pdf.encode_txt(f"PLANO DE CLASSIFICAÇÃO DE DOCUMENTOS"), 0, 1, 'C')
    ...
    for _, cod, tipo, txt in dados_ordenados:
        ...
        pdf.multi_cell(190, 6, pdf.encode_txt(f"{indent}{cod} - {txt} [{tipo}]"))
...
if menu == "Área do Aluno":
    ...
    if 'aluno_logado' not in st.session_state:
        ...
        orgao_input = st.text_input("Nome da Instituição / Órgão do seu plano:", key="input_orgao").strip() if opcao_acesso == "Primeiro Acesso (Criar Novo Perfil)" else ""
        ...
        with tab4:
            ...
            if dados_pdf:
                try:
                    pdf_data = gerar_relatorio_final(st.session_state['aluno_orgao'], st.session_state['aluno_nome'], st.session_state['aluno_matricula'], membros_lista, dados_pdf)
